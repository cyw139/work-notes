...
- [ ] --force-search-engine-choice-screen ⊗	Force-enable showing the search engine choice screen for testing regardless of region or choice already having been made. ↪
...
- [ ] --glanceables-key ⊗	No description ↪
- [ ] --gles ⊗	No description ↪
...
- [ ] --h[13] ⊗	No description ↪
- [ ] --hardware-video-decode-framerate[25] ⊗	Some (Qualcomm only at the moment) V4L2 video decoders require setting the framerate so that the hardware decoder can scale the clocks efficiently. This provides a mechanism during testing to lock the decoder framerate to a specific value. ↪
- [ ] --hardware_video_decoding[26] ⊗	No description ↪
...
- [ ] --initial-preferences-file[27] ⊗	Manually sets the initial preferences file. This is required to change the initial preferences when the default file is read-only (eg. on lacros). Passing this flag will reset the preferences regardless of whether this is the first run. ↪
...
- [ ] --ipc-dump-directory[28] ⊗	Dumps IPC messages sent from renderer processes to the browser process to the given directory. Used primarily to gather samples for IPC fuzzing. ↪
- [ ] --ipc-fuzzer-testcase[28] ⊗	Specifies the testcase used by the IPC fuzzer. ↪
...
- [ ] --load-guest-mode-test-extension[6] ⊗	Path to the unpacked test extension to load into guest mode. The extension ID must match kGuestModeTestExtensionId. ↪
- [ ] --load-signin-profile-test-extension[6] ⊗	Path to the unpacked test extension to load into the signin profile. The ID extension loaded must match kTestSigninProfileExtensionId. ↪
...
- [ ] --log-on-ui-double-background-blur[30] ⊗	Checks and logs double background blur as an error if any. ↪
...
- [ ] --no-appcompat-clear[1] ⊗	Don't try to clear downlevel OS appcompat layers out of Chrome's AppCompatFlags\Layers value in the Windows registry on process startup; see https://crbug.com/1482568. ↪
...
- [ ] --offscreen-document-testing ⊗	Allows the use of the `testing` reason in offscreen documents. ↪
...
- [ ] --package-version-code[10] ⊗	No description ↪
...
- [ ] --print_backend[32] ⊗	No description ↪
...
- [ ] --profile-base-name[29] ⊗	Pass the basename of the profile directory to specify which profile to get information. Only relevant when used with - [ ] --list-apps switch. ↪
...
- [ ] --profile-management-attributes[29] ⊗	Domains and associated SAML attributes for which third-party profile management should be enabled. Input should be in JSON format. ↪
...
- [ ] --restore-key-on-lock-screen ⊗	Indicates that the cryptohome keys are evicted and lock screen should message cryptohomed to run full authentication and restore filesystem keys. ↪
...
- [ ] --screen_ai[33] ⊗	No description ↪
- [ ] --seal-key ⊗	Supply secret key for Seal feature. ↪
- [ ] --search-engine-choice-country ⊗	Override the country used for search engine choice region checks. Intended for testing. Expects 2-letter country codes. ↪
...
- [ ] --shared-array-buffer-unrestricted-access-allowed[11] ⊗	Enables shared array buffer on desktop, gated by an Enterprise Policy. TODO(crbug.com/1144104) Remove when migration to COOP+COEP is complete. ↪
...
- [ ] --start-stack-profiler-with-java-name-hashing[10] ⊗	Hash names of Java functions in SSM. Only has an effect if SSM is enabled. ↪
...
- [ ] --system-aec-enabled[34] ⊗	Enforce system audio echo cancellation. ↪
...
- [ ] --time-zone-for-testing ⊗	The time zone to use for testing. Passed to renderers and plugins on startup. ↪
...
- [ ] --use-cras[34] ⊗	Use CRAS, the ChromeOS audio server. ↪
...
- [ ] --use-gpu-high-thread-priority-for-perf-tests ⊗	Changes the type (to kRealtimeAudio) of gpu process and compositor thread. This is only to be used for perf tests on macOS for more reliable values. ↪
...
- [ ] --use-system-default-printer[35] ⊗	Uses the system default printer as the initially selected destination in print preview, instead of the most recently used destination. ↪
- [ ] --use-system-proxy-resolver ⊗	Uses WinHttp to resolve proxies instead of using Chromium's normal proxy resolution logic. This is only supported in Windows. TODO(https://crbug.com/1032820): Only use WinHttp whenever Chrome is exclusively using system proxy configs. ↪
...
- [ ] --user-data-migrated[36] ⊗	Indicates that this process is the product of a relaunch following migration of User Data. ↪
...
- [ ] --vulkan-heap-memory-limit-mb ⊗	Specifies the heap limit for Vulkan memory. TODO(crbug/1158000): Remove this switch. ↪
- [ ] --vulkan-null ⊗	No description ↪
- [ ] --vulkan-sync-cpu-memory-limit-mb ⊗	Specifies the sync CPU limit for total Vulkan memory. TODO(crbug/1158000): Remove this switch. ↪
...
- [ ] --web-sql-access ⊗	Controls whether WebSQL is force enabled. ↪
...
- [ ] --webapk-server-url[37] ⊗	Custom WebAPK server URL for the sake of testing. ↪
...
- [ ] --webview-disable-app-recovery ⊗	Disables WebView from checking for app recovery mitigations. ↪
...
- [ ] --webview-enable-app-recovery ⊗	Enables WebView to check for app recovery mitigations. ↪
